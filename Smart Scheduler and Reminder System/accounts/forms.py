...
class SignupForm(UserCreationForm):
    ...
    is_student = forms.BooleanField(widget=forms.RadioSelect(attrs={
        ...
        required=False,
        label='Are you a student or lecturer/Admin?',
    )
...
class ReminderSettingsForm(forms.ModelForm):
    reminder_preference = forms.MultipleChoiceField(
        choices=User.REMINDER_TIME_CHOICES,
        widget=forms.CheckboxSelectMultiple,
        label="Reminder Times",
        help_text="Select all desired reminder times."
    )

    class Meta:
        model = User
        fields = ['reminder_preference', 'notification_method']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            # Set the initial value for the multiple choice field
            self.fields['reminder_preference'].initial = self.instance.reminder_preference.split(',')

    def save(self, commit=True):
        instance = super(ReminderSettingsForm, self).save(commit=False)
        # Join the list of choices into a comma-separated string
        instance.reminder_preference = ','.join(self.cleaned_data['reminder_preference'])
        if commit:
            instance.save()
        return instance
