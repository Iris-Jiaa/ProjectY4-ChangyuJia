from .forms import SignupForm, EditProfileForm, EditFacultyDetailsForm, EditStudentDetailsForm, StudentRegistrationForm, FacultyRegistrationForm, ReminderSettingsForm
...
def show_faculty_registration_form(wizard):
    """
        ...
        If the user selects radio button 'Admin/Lecturer', show FacultyRegistrationForm.
    """
...
class EditStudentProfileView(View):
    ...
    reminder_form_class = ReminderSettingsForm
    ...
    def get(self, request, _student_name, *args, **kwargs):
        ...
        reminder_form = self.reminder_form_class(instance=request.user)

        context = {
            'EditProfileForm': editprofile_form,
            'UpdateStudentInfoForm': editstudent_form,
            'ReminderSettingsForm': reminder_form,
        }
        return render(request, self.template_name, context)
    ...
    def post(self, request, _student_name, *args, **kwargs):
        ...
        reminder_form = self.reminder_form_class(request.POST, instance=request.user)
        ...
        if reminder_form.is_valid():
            reminder_form.save()
            messages.info(request, 'Reminder settings updated successfully')
            return redirect('student_profile', _student_name)
